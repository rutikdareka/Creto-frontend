import React from "react";
import { Link } from "react-router-dom";
import { useNavigate } from "react-router-dom";
import Box from "@mui/material/Box";

function Inform() {
  return (
    <>
      <div className="login_main_wrapper">
        <div className="login_authContent">
          <div className="login_sub_wrapper">
            <Box
              sx={{
                height: 400,
                display: "flex",
                flexDirection: "column",
                gap: "1rem",
              }}
              className="sdsdd"
            >
              <div style={{ textAlign: "center", height: "164px" }}>
                <img
                  style={{ height: "200px", width: "200px" }}
                  src="data:image/png;base64,"
                  alt=""
                />
              </div>
              <div
                style={{
                  textAlign: "center",
                  fontSize: "24px",
                  fontWeight: "700",
                }}
              >
                Check your Inbox!
              </div>{" "}
              <br />
              <div
                style={{
                  textAlign: "center",
                  fontSize: "24px",
                  fontWeight: "500",
                }}
              >
                We've sent you a link for password creation or reset
              </div>
              <br /> <br />
              <div className="fghf" style={{ textAlign: "center" }}>
                Haven't received an email? <br />
                <Link
                  style={{ color: "#3e92f2", textDecoration: "none" }}
                  to={"/send-mail"}
                >
                  Resend Email?
                </Link>
              </div>
            </Box>
          </div>
        </div>
      </div>
    </>
  );
}

export default Inform;
